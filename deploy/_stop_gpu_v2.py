"""Stop GPU miners — find webcoin module by name, set _running=False, kill processes."""
...
CODE = (
    ...
    "    if 'webcoin' not in mod_name:\n"
    "        continue\n"
    "    mod = sys.modules[mod_name]\n"
    "    ref = None\n"
    "    try:\n"
    "        ref = getattr(mod, '_dashboard_ref', None)\n"
    ...
    "                    gm._process.kill()\n"
    "                    lines.append('killed gpu via dashboard pid=' + str(gm._process.pid))\n"
    "                gm._process = None\n"
    "                stopped = True\n"
    "                lines.append('watchdog stopped via ' + mod_name)\n"
    "    except Exception as e:\n"
    "        lines.append('err on ' + mod_name + ': ' + str(e)[:60])\n"
    "if not stopped:\n"
    "    lines.append('no dashboard_ref found, brute-forcing module scan')\n"
    "    for mod_name in list(sys.modules.keys()):\n"
    "        if 'webcoin' not in mod_name and 'gpu_miner' not in mod_name:\n"
    "            continue\n"
    ...
    "                if (hasattr(obj, '_running') and hasattr(obj, 'miner_type')\n"
    "                        and hasattr(obj, '_process') and hasattr(obj, '_watchdog')):\n"
    "                    obj._running = False\n"
    "                    if obj._process and obj._process.poll() is None:\n"
    "                        obj._process.kill()\n"
    "                        lines.append('killed via ' + mod_name + '.' + attr_name)\n"
    "                    obj._process = None\n"
    "                    stopped = True\n"
    ...
    "r = subprocess.run(['ps', '-eo', 'pid,stat,args'], capture_output=True, text=True, timeout=10)\n"
    "live = []\n"
    "for l in r.stdout.strip().split(chr(10))[1:]:\n"
    "    if any(n in l for n in kill_names):\n"
    "        parts = l.strip().split(None, 2)\n"
    "        if len(parts) >= 2 and 'Z' not in parts[1]:\n"
    "            live.append(l.strip()[:120])\n"
    "if live:\n"
