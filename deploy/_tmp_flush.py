...
for cmd in [
    ...
    lines.append(f'{" ".join(cmd)} -> rc={r.returncode} {r.stderr.strip()[:100]}')
    if r.returncode == 0:
        break
...
for pool in ['rvn.2miners.com', 'gulf.moneroocean.stream', 'etchash.unmineable.com', 'donate.xmrig.com']:
    try:
...
# TCP connect test
for host, port in [('rvn.2miners.com', 16060), ('gulf.moneroocean.stream', 443)]:
    try:
