"""
GPU miner management — uses T-Rex for Ravencoin (KAWPOW) on 2Miners,
paid out in RVN. Works on NVIDIA GPUs.
...
def fetch_trex_http_summary(api_port: int) -> dict | None:
    """Read T-Rex's JSON from its local HTTP API.

    Used by the dashboard when T-Rex was started outside orchestration (e.g. SRL deploy)
...
def _detect_gpus_windows_wmi() -> list[dict]:
    ...
    for raw in names_out.strip().splitlines():
        ...
        if not is_good:
            continue
        # index -1: let T-Rex pick devices (WMI order != CUDA index)
...
class GPUMinerManager:
    def __init__(self, base_dir: Path | str | None = None, bin_dir: Path | str | None = None):
        ...
        self.miner_type = "trex"
        # CUDA indices from nvidia-smi (only these devices get T-Rex)
    ...
    def _build_cmd(self) -> list[str]:
        ...
        # T-Rex uses simple wallet.worker format
        user_str = f"{self.wallet}.{self.worker}"
        
        # Build pool URL with optional TLS
        if self.tls:
            pool_str = f"stratum+ssl://{self.pool}:{self.port}"
        else:
            pool_str = f"stratum+tcp://{self.pool}:{self.port}"
    ...
    def get_summary(self) -> dict | None:
        """Fetch raw JSON from T-Rex's local HTTP API."""
        data = fetch_trex_http_summary(self.api_port)
        if data is not None:
